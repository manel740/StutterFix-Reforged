...
public class StutterFix {
	...
	public static boolean removeYieldOption = !ModList.get().isLoaded("vulkanmod");
	...
	public static int getDefaultStutterFixMainWorkerExecutorCount(){
		...
		int vanillaThreadsCount = Mth.clamp(availableProcessors - 1, 1, StutterFix.getMaxBackgroundThreads());
	...
	public static int getDefaultVanillaMainWorkerExecutorCount(){
		int availableProcessors = Runtime.getRuntime().availableProcessors();
		return Mth.clamp(availableProcessors - 1, 1, StutterFix.getMaxBackgroundThreads());
	}
